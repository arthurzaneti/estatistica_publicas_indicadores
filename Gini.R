...
teste <- gini(c(0,0.2,0.4,0.6,0.8,1))
glue("This should be 0: {teste}")

# Using the function in our data, manually inputted sadly
# All of them stand for Metropolitan Region... (I'm gonna the region by the side of each line)
RMPA <- gini(c(0,0.02,0.05,0.09,0.14,0.20,0.27,0.36,0.47,0.64,1)) # Porto Alegre
RMSP <- gini(c(0,0.01,0.04,0.08,0.13,0.18,0.25,0.33,0.44,0.6,1)) # São Paulo
RMSA <- gini(c(0,0.01,0.04,0.07,0.11,0.15,0.21,0.29,0.39,0.56,1)) # Salvador 
RMRE <- gini(c(0,0.01,0.04,0.08,0.12,0.17,0.23,0.31,0.41,0.56,1)) # Recife
RMBH <- gini(c(0,0.02,0.05,0.08,0.12,0.18,0.24,0.32,0.43,0.6,1)) # Bahia
...
RMPA_esc_df <-  data.frame(gini = c(0.367, 0.385, 0.362), escolaridade, source = "RMPA")
View(RMPA_esc_df)
